---
title: "Security Testing and Cyber-Security: Use Case Simulated Man in The Middle Attack and Prevention"
...
- Security Breaches: A security breach happen when a system fail in its security processes and cause unauthorized party access to the system. A security breach can happen by software problems (error, fault, failures), outdated security technology, ransom ware, malware ...
...
Popular tools that is used in Man-In-The_Middle Attack:
- MAC Address Changer
- Network Scanner
...
A private Network can only access by devices within its network. All tools will be write from scratch and source code can be found at [13]. Please take look at Pre-requirement
section if you not sure about topics that I mention in the next sections. The benefit of writing script attack is that you can schedule the whole script as one and automate the process. 
...
can be established just by one computer being hack, and spread out to many other devices within the network. Depend on the size of the attack, it can cause millions lost.  
...
spread out the virus after gaining access and control the system.
...
devices within the network. Some virus can contain themselves, create back door, and by pass security scanner by changing its MAC 
...
- Step 4: Modify Data, spread virus
	...
	- Using 'DNS Spoofer' (modify data in DNS Layer) to redirect the destination on the computer on network (e.g. to a fake website) so that hacker can install a backdoor on other local computer.
